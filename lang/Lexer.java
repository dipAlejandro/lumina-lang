   ...
   private void readIdentifierOrKeyword() {
    ...
    private void readSymbol() {
        ...
        switch (c) {
            ...
            case '!' -> {
                if (peek(0) == '=') { pos++; tokens.add(new Token(TokenType.NEQ, "!=", line)); }
                else tokens.add(new Token(TokenType.NOT, "!", line));
            }
            case '<' -> {
                if (peek(0) == '=') { pos++; tokens.add(new Token(TokenType.LTE, "<=", line)); }
                else tokens.add(new Token(TokenType.LT, "<", line));
            }
            case '>' -> {
                if (peek(0) == '=') { pos++; tokens.add(new Token(TokenType.GTE, ">=", line)); }
                else tokens.add(new Token(TokenType.GT, ">", line));
            }
            case '&' -> { if (peek(0) == '&') { pos++; tokens.add(new Token(TokenType.AND, "&&", line)); } }
            case '|' -> { if (peek(0) == '|') { pos++; tokens.add(new Token(TokenType.OR,  "||", line)); } }
